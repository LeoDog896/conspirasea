...
{#if visible}
  <div on:click={() => { visible = !visible }} transition:fade class="fixed z-10 top-0 left-0 w-screen h-screen bg-gray-600/50"></div>
  <div transition:fly={{ x: -150 }} class="rounded-r-lg text-center fixed z-20 top-0 left-0 bg-white min-h-screen shadow-lg p-5">
    {#each $conspiraseas as conspirasea, i}
      <div class="bg-gray-100 rounded-lg shadow-lg p-4 mt-4">
        {#if conspirasea.editable}
          <input on:blur={() => { conspirasea.editable = false }} placeholder="Name" bind:value={conspirasea.name} />
        {:else}
          <p on:dblclick={() => { conspirasea.editable = !conspirasea.editable }}>{conspirasea.name}</p>
        {/if}
        <div>
          <button 
    ...
    <button
      ...
      on:click={() => $conspiraseas = [...$conspiraseas, { name: "New Conspirasea", elements: [] }]}
    >
